...
if ( ! class_exists( 'Astra_Target_Rules_Fields' ) ) {
 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
						...
						switch ( $rule_case ) {
							...
							case 'specifics':
								if ( isset( $rules['specific'] ) && is_array( $rules['specific'] ) ) {
									$apply_on_child = isset( $rules['apply_on_child'] ) && $rules['apply_on_child'];
									...
									foreach ( $rules['specific'] as $specific_page ) {
										...
										if ( 'post' === $specific_post_type ) {
											...
											// we're checking if the current page is a child of THIS specific selected page
											if ( $apply_on_child && $this->is_child_of_post( $post_id, $specific_post_id ) ) {
												$display = true;
												break;
											}
			...
			if ( false === $meta_header ) {
				...
				switch ( $current_page_type ) {
					...
					case 'is_singular':
						...
						if ( $current_post_obj && $current_post_obj->post_parent ) {
							$parent_ids = array();
							$parent_id  = $current_post_obj->post_parent;
							while ( $parent_id ) {
								$parent_ids[] = $parent_id;
								$parent_obj   = get_post( $parent_id );
								$parent_id    = $parent_obj ? $parent_obj->post_parent : 0;
							}

							if ( ! empty( $parent_ids ) ) {
								foreach ( $parent_ids as $pid ) {
									$meta_args .= " OR (pm.meta_value LIKE '%\"specifics\"%' AND pm.meta_value LIKE '%\"apply_on_child\";b:1%' AND pm.meta_value LIKE '%\"post-{$pid}\"%')";
								}
							}
						}
		 ...
		 *
		 * @since  1.0.0
		 * @param  int $current_post_id Current post/page ID.
		 * @param  int $parent_post_id  Parent post/page ID to check against.
		 *
		 * @return bool True if current post is a child/descendant of parent post, false otherwise.
		 */
		...
		public static function get_format_rule_value( $save_data, $key ) {
			...
			if ( isset( $save_data[ $key ]['rule'] ) ) {
				...
				foreach ( $save_data[ $key ] as $meta_key => $value ) {
					if ( 'apply_on_child' === $meta_key ) {
						$meta_value[ $meta_key ] = ! empty( $value ) ? true : false;
					} elseif ( ! empty( $value ) ) {
