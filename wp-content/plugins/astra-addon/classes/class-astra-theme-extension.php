...
if ( ! class_exists( 'Astra_Theme_Extension' ) ) {
	...
	class Astra_Theme_Extension {
		...
		public function __construct() {
			...
			add_action( 'pll_init', array( $this, 'polylang_compatibility' ) );
		...
		public function action_links( $links = array() ) {
			...
			if ( ASTRA_ADDON_BSF_PACKAGE && is_callable( 'BSF_License_Manager::bsf_is_active_license' ) ) {
				$product_id        = is_callable( 'bsf_extract_product_id' ) ? call_user_func( 'bsf_extract_product_id', ASTRA_EXT_DIR ) : '';
				$is_license_active = call_user_func( 'BSF_License_Manager::bsf_is_active_license', $product_id );

				if ( ! $is_license_active ) {
					$license_url                      = esc_url( admin_url( $admin_base . '?page=' . $slug . '&path=settings' ) );
					$action_links['activate_license'] = '<a href="' . $license_url . '" class="astra-addon-license-status-link" aria-label="' . esc_attr__( 'Activate Astra Pro license', 'astra-addon' ) . '" style="color: #851C45; font-weight: 600;">' . esc_html__( 'Activate License', 'astra-addon' ) . '</a>';
				}
			}
		...
		public function polylang_compatibility() {
			...
			require_once ASTRA_EXT_DIR . 'compatibility/class-astra-polylang-compatibility.php';
		}
