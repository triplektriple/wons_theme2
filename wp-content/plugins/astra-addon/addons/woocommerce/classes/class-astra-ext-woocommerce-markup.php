...
if ( ! class_exists( 'ASTRA_Ext_WooCommerce_Markup' ) ) {
 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function add_registration_link_text() {
			...
			printf(
				'<p class="ast-woo-form-actions">
					%1$s
					<a href="#createaccount" data-type="do-register" class="ast-woo-account-form-link">
						%2$s
					</a>
				</p>',
		...
		public function product_flip_image() {
			...
			if ( 'swap' === $hover_style ) {
				...
				if ( $attachment_ids ) {
					...
					echo apply_filters( 'astra_woocommerce_product_flip_image', wp_get_attachment_image( reset( $attachment_ids ), $image_size, false, array( 'class' => 'show-on-hover' ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_get_attachment_image() handles escaping and sanitization by default; escaping is not required.
				}
			}
		}
		...
		public function shop_js_localize( $localize ) {
			...
			$localize['shop_total_products']        = $wp_query->found_posts;
			...
			$localize['shop_result_count_format']   = apply_filters(
				'astra_shop_result_count_format',
				sprintf(
					/* translators: 1: current count, 2: total results */
					__( 'Showing 1–%1$s of %2$s results', 'astra-addon' ),
					'{count}',
					'{total}'
				)
			);
	...
	<div class="product-links">
		...
		private function get_adjacent_products( $args ) {
			if ( ! empty( $args['title_query'] ) ) {
				...
				return $wpdb->get_col(
					$wpdb->prepare(
						// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $compare and $order are validated above.
						"SELECT ID FROM {$wpdb->posts} WHERE post_type = %s AND post_status = 'publish' AND post_title {$compare} %s ORDER BY post_title {$order} LIMIT %d",
						'product',
						$title,
						$posts_per_page
					)
		...
		public function ast_load_product_quick_view_ajax() {
			...
			if ( ! isset( $_REQUEST['product_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is not required for this simple check.
				die;
			}

			$product_id = intval( $_REQUEST['product_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is not required for this simple parameter retrieval.
		...
		public function woo_single_product_gallery_output() {
			/**
			 * Filter to allow child themes or plugins to override the single product gallery template.
			 *
			 * @param string $template_path Path to the single product gallery template.
			 * @return string The template path to use.
			 * @since 4.11.12
			 */
			$template_path = apply_filters( 'astra_addon_single_product_gallery_template', ASTRA_ADDON_EXT_WOOCOMMERCE_DIR . '/templates/single-product-gallery.php' );
		...
		public function woo_product_tabs_layout_output() {
			/**
			 * Filter to allow child themes or plugins to override the single product tabs template.
			 *
			 * @param string $template_path Path to the single product tabs template.
			 * @return string The template path to use.
			 * @since 4.11.12
			 */
			$template_path = apply_filters( 'astra_addon_single_product_tabs_template', ASTRA_ADDON_EXT_WOOCOMMERCE_DIR . '/templates/single-product-tab-layout.php' );
			include_once $template_path;
		}
		...
		public function astra_add_cart_single_product_quantity() {
			...
			$threeball_product_quantity = isset( $_POST['quantity'] ) ? apply_filters(
				...
				apply_filters(
					...
					// phpcs:ignore Generic.PHP.ForbiddenFunctions.FoundWithAlternative -- Safe: preg_replace without /e modifier, sanitizes numeric input.
					preg_replace(
						'/[^0-9\.]/',
						'',
