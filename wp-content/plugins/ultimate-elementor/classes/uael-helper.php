	...
	public static function get_active_widget_stylesheet() {
		...
		foreach ( UAEL_Config::$widget_list as $key => $block ) {
			...
			switch ( $block_name ) {
				...
				case 'Woo_Thank_You':
					if ( ! isset( $is_already_woo_thank_you ) || ! $is_already_woo_thank_you ) {
						$combined['uael-woo-thank-you'] = array(
							'path'     => 'assets/' . $folder . '/modules/woo-thank-you' . $suffix . '.css',
							'path-rtl' => 'assets/min-css/modules/woo-thank-you-rtl.min.css',
							'dep'      => array(),
						);
						$is_already_woo_thank_you       = true;
					}
					break;
