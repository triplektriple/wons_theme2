( function( $ ) {
	...
	UAELRegistrationForm = {
		...
		_submitForm: function( $this, widget_wrapper, $scope ) {
			...
			// fire before‐submit event
			const triggerEvent = new CustomEvent('uael_on_trigger_registration_submission', {
				cancelable: true,
				detail: { form: form_wrap, scope: $scope, user_data }
			});

			if ( ! document.dispatchEvent(triggerEvent) ) {
				loader.classList.remove('uael-active');
				return;
			}
