( function( $ ) {
	...
	var WidgetUAELGoogleMapHandler = function ( $scope, $ ) {
		...
		// Wait for marker library to load if using Advanced Markers
		if ( typeof google.maps.marker === 'undefined' ) {
			// Retry after a short delay to allow marker library to load
			setTimeout(function() {
				WidgetUAELGoogleMapHandler( $scope, $ );
			}, 100);
			return;
		}
		...
		var selector                = $scope.find( '.uael-google-map' ).eq(0),
			...
			custom_map_id           = ( selector.data( 'map-id' ) != '' ) ? selector.data( 'map-id' ) : 'DEMO_MAP_ID',
			advance_marker          = selector.data( 'advance-marker' ),
			i                       = '',
		...
		( function initMap () {
			...
			// Google Maps doesn't allow both styles and mapId to be set
			// Prioritize advance_marker setting over map styles
			if ( advance_marker === 'yes' && typeof google.maps.marker !== 'undefined' && google.maps.marker.AdvancedMarkerElement ) {
				// Use Advanced Markers when enabled - set mapId and ignore custom styles
				map_options.mapId = custom_map_id; // Use custom mapId if provided, otherwise defaults to DEMO_MAP_ID
			} else if ( map_style && map_style.length > 0 ) {
				// Use custom styles only when advance_marker is not enabled
			...
			// Import the AdvancedMarkerElement library if available
			...
			for ( i = 0; i < locations.length; i++ ) {
				...
				if ( '' != lat.length && '' != lng.length ) {
					...
					// Use AdvancedMarkerElement if advance_marker is enabled, fallback to legacy Marker
					var marker;
					if (advance_marker === 'yes' && AdvancedMarkerElement) {
						...
						// Apply custom icon for AdvancedMarkerElement
						if (icon && icon.url) {
						...
						// Apply animation for AdvancedMarkerElement
						if (animation) {
							if (animation === google.maps.Animation.DROP) {
								// Simulate drop animation with CSS
								setTimeout(() => {
									if (marker.element) {
										marker.element.style.animation = 'uael-marker-drop 0.3s ease-out';
									}
								}, i * 100);
							} else if (animation === google.maps.Animation.BOUNCE) {
								// Apply bounce animation
								setTimeout(() => {
									if (marker.element) {
										marker.element.style.animation = 'uael-marker-bounce 2s infinite';
									}
								}, 100);
							}
						}
